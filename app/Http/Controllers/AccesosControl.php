...
class AccesosControl extends BaseSoapController
{
  ...
  ->join('Pisos', 'Pisos.idEdificio', '=', 'Edificios.idEdificio')
    ...
    public function store($id, Request $request)
    {
        ...
        try {
            ...
            if ($validator->fails()) {
               ...
               // $this->imprimir($datos);
                ...
                $Auditoria->Accion = "La persona: " . $Personas->Nombres . " - " . $Personas->Cedula . " Ha visitado la organizacion " . $datos->nombre_organizacion;
                $Auditoria->save();
                DB::commit();
                $guardo = true;
            }
        ...
        if ($guardo) {
            try {
                $impresora=ImpresorasModelo::findOrFail(\Auth::user()->idImpresora);
                $datos->nombre_impresora=$impresora->Nombre;
                $this->imprimir($datos);
                $datos->tipo = 0;
                if (PermisosModelo::where('idUsuario', '=', \Auth::user()->id)->where('idRol', '=', 1)->count() == 0) {
                    $datos->tipo = 2;
                ...
                return response()->json(array('data' => $datos, 'errors1' => array('txt_error_impresora' => $ex->getMessage())));
            }
        }
        return response()->json(array('errors' => $e->getMessage()));
    }

    private function imprimir($datos)
    {
